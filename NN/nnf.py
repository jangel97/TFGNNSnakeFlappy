...
class NN:
    ...
    def save_data(self,request,get_information,wasGoodAction): #requestJson(obs,action, wasGoodAction,done)
	#print "LOKO " + str(request)
	print request["obs"]
	food_distance=None
	if request["done"]:
		self.training_data.append([self.add_action_to_observation(request["obs"], request["action"]), -1])	
	else:
		#food_distance=get_information(request);
		good=wasGoodAction(request)
		if good:	#ESTO ES LO Q HAY Q TOCAAAR
			self.training_data.append([self.add_action_to_observation(request["obs"], request["action"]), 1])
		elif good==False:	#
			self.training_data.append([self.add_action_to_observation(request["obs"], request["action"]), 0])	#aqui habia un 0
